...
class TestLocalDrillForms(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        # Cleanup dummy data generated by this test suite
        from tracker.db import get_db_connection
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("DELETE FROM pitches WHERE vacancy_id LIKE 'vac_drill_%'")
        cur.execute("DELETE FROM agent_sessions WHERE vacancy_id LIKE 'vac_drill_%'")
        cur.execute("DELETE FROM application_history WHERE vacancy_id LIKE 'vac_drill_%'")
        cur.execute("DELETE FROM vacancies WHERE id LIKE 'vac_drill_%' OR company = 'Acme Corp'")
        conn.commit()
        conn.close()
